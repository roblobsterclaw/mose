...
- **Filer roster**: `cik-map.json` carries `status` — `approved` votes, `pruned`/`dormant` keep history but are excluded from holdings, consensus and eligibility by `votes_now()` in the builders — and, since 14 Sep 2026, **`wing`**: `core` or `bench`. **Roster is 141 entries / 121 voting: 41 Core, 80 Bench** (Bancroft and **Guy Spier** dormant-core — Spier's real filer is Aquamarine Zurich AG 1953324, added 17 Sep 2026 when Joe's 15-superinvestor check showed the roster had the wrong Aquamarine (a Cayman LatAm book, now pruned); his fund is winding down since 28 Mar 2026 so it is tracked, not voting. **Howard Marks / Oaktree** 949509 added 17 Sep as Bench — credit house, 13F is a sliver, the memos are the signal. `pull_sec_13f_history.py --only <ciks>` pulls new filers via the submissions API and merges into the existing file, no 11-index download; 17 pruned incl. 9 Sep: Aschenbrenner, Whale Rock, Burry; 12 Sep: Wellcome, Allen Holding, Hikari, Milestone, Cathie Wood; 14 Sep: Gavin Baker, Gerstner, Laffont, Coleman, Niles, Halvorsen, Foxhaven, Manitou, JIA). Joe reviewed every name on 14 Sep 2026 in the Voter Roster Review artifact (`https://claude.ai/code/artifact/dfae62ae-029c-4a8e-80c7-9570d6456161`, decisions in its `db` collection `decisions/<cik>`) — 89 filers added, and after dossiers he moved Giverny, Dorsey and Ruane Cunniff to Core and kept **MayTech** Core despite my flag that it is a self-described global-growth shop. **`cik-map.json` is authoritative for `status` and `wing`** — all three builders overlay it via `roster_status()` / `roster_wing()` at load time, because `data/sec-13f-filings.json` only carries a copy from pull time.
- **Core vs Bench (the voting model, Joe's call 14 Sep 2026):** Core voters set the top 15; Bench voters only feed the watch rails. `build_consensus_history.py` (schema v2) writes per-row `core_now/core_prior/core_yr_ago/core_series/core_holders/core_sum_pct/core_avg_pct/bench_now` plus top-level `core_15`, `all_15`, `agree`, `bench_watch`, `core_count`, `bench_count`. Conviction score = holders × avg % of book = `sum_pct`, **with any one filer capped at 25 points** (`CAP_PCT`, Joe's call 14 Sep — one all-in book can't buy a seat); a name needs ≥3 holders in the wing to rank. The list shown is the **Core 25** (`core_top`/`all_top`, `TOP_N`) with a line after 15; `core_15`/`all_15` are kept as the first 15. Joe holds fewer names than the list — the list is the map, the Buy tab is what he owns. The Consensus tab renders the **Core check** (four blocks: Core 15 vs buy list with IBKR $ owned; core movement this quarter; Core 15 vs All-in 15; bench watch) and a Core/All-in rank toggle. The **Investors tab is roster-driven** (`investorCards()` = cik-map + dossiers + each filer's top positions from holdings-latest.json; `INVESTORS_DATA` survives only as an annotation layer and for non-filers). `build_holdings_from_13f.py` resolves raw CUSIPs through cusip-map (was silently dropping 3/4 of Einhorn/Pabrai).
- **Dossiers**: `reference-data/investor-dossiers.json` → `{"firms": {cik: {...}}}` — **a dict keyed by CIK, never a list** (a save that treated it as a list wiped 91 entries on 14 Sep; restored). 100 firms; every Core voter has one; `mose_note` carries the Bench/Core verdict. Joe wants an **annual dossier refresh that captures each manager's letters** — not built yet.
