...
- **Core vs Bench (the voting model, Joe's call 14 Sep 2026):** Core voters set the top 15; Bench voters only feed the watch rails. `build_consensus_history.py` (schema v2) writes per-row `core_now/core_prior/core_yr_ago/core_series/core_holders/core_sum_pct/core_avg_pct/bench_now` plus top-level `core_15`, `all_15`, `agree`, `bench_watch`, `core_count`, `bench_count`. Conviction score = holders × avg % of book = `sum_pct`; a name needs ≥3 holders in the wing to rank. The Consensus tab renders the **Core check** (four blocks: Core 15 vs buy list with IBKR $ owned; core movement this quarter; Core 15 vs All-in 15; bench watch) and a Core/All-in rank toggle. The **Investors tab is roster-driven** (`investorCards()` = cik-map + dossiers + each filer's top positions from holdings-latest.json; `INVESTORS_DATA` survives only as an annotation layer and for non-filers). `build_holdings_from_13f.py` resolves raw CUSIPs through cusip-map (was silently dropping 3/4 of Einhorn/Pabrai).
