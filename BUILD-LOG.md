...
**16 Sep 2026:** Joe reported the architecture table on his device showing no Toll booths and no Dry powder. The build renders all four buckets with default data (verified headless), so the gap is in his synced `targetsData` copy. Added a **bucket self-heal** in `migrateTargets`: the four canonical buckets must exist in canonical order and Toll booths / Dry powder must not be empty — a missing one is re-inserted with its canonical names (minus names already in another bucket), an empty toll/dry is refilled, nothing is removed, and the repaired copy is pushed to the cloud (`healedAt` stamped). `APP_BUILD 2026-09-16a`.

**16 Sep 2026, root cause from Joe's screenshot:** Toll booths had 0 names, Dry powder only RSP/VO, Other positions 48 names and $1.16M. Cause: the ⠿ drag handle (`touch-action:none`, drag armed on pointerdown, auto-scroll at the screen edge) turned iPad thumb-scrolls into drags that dropped rows into Other positions at the bottom. Fix `2026-09-16b`: touch/pen must press-and-hold 450 ms with the finger still before a drag arms (`targDragBegin`), handle allows vertical panning; **v21** one-time repair moves every canonical Forever / Toll booths / Dry powder name back out of Other (and re-adds any that vanished), logging to `targetsData.healLog` with a toast and a line under ⚙️. Tested headless against a copy matching the screenshot: 13 / 4 / 6 names restored, Joe's own Other names untouched, quick touch-move does not drag, 600 ms hold does.
